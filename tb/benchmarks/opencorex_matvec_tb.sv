module opencorex_matvec_tb;
    ...
    // Keep the core direct in this baseline test so arbitration remains
    // outside the measured CPU-plus-adapter path.
    opencorex_core dut (
        .clk           (clk),
        .reset         (reset),
    ...
    initial begin : run_test
        ...
        for (int cycle = 0; cycle < MAX_CYCLES; cycle++) begin
            ...
            if (memory_write_enable && (memory_address == DONE_ADDR)) begin
                ...
                $display(
                    "PASS: direct-core matrix-vector benchmark completed at cycle %0d | active cycles=%0d",
                    cycle,
                    cycle + 1
                );
                $finish;
